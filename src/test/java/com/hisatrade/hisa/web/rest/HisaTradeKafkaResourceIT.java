...
import com.hisatrade.hisa.config.KafkaSseConsumer;
import com.hisatrade.hisa.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class HisaTradeKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hisa-trade-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hisa-trade-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/hisa-trade-kafka/unregister"));
                return;
            }
        }
